...
TUNED_METHODS = ("prompt_tuning", "prefix_tuning")  # compare each vs Base
LABELS = ["no", "yes"]
OUTPUT_CSV = os.path.join(PROJECT_OUT, "stats_eval_test.csv")
...
def _bootstrap_diff(y_true, y_base, y_tuned, B=5000, seed=0):
    ...
    for b in range(B):
        ...
        fb, ft = _macro_f1(yt,yb), _macro_f1(yt,yu)
        acc_b[b], acc_t[b], acc_d[b] = ab, at, (at-ab)
        f1_b[b],  f1_t[b],  f1_d[b]  = fb, ft, (ft-fb)
    def ci(a):
        lo, hi = np.quantile(a, [0.025, 0.975])
        return float(a.mean()), float(lo), float(hi)
    return {"acc_base":ci(acc_b), "acc_tuned":ci(acc_t), "acc_diff":ci(acc_d),
            "f1_base":ci(f1_b),   "f1_tuned":ci(f1_t),   "f1_diff":ci(f1_d)}

def _load_preds_csv(path):
    # expects columns: id, final_decision, prediction
    ...
    def ny(x):
        ...
        return "no"
    df["final_decision"] = df["final_decision"].map(lambda s: "yes" if str(s).strip().lower()=="yes" else "no")
    df["prediction"] = df["prediction"].map(ny)
    return df

def compare_one(split, tuned_method, seed=0):
    ...
    boots = _bootstrap_diff(y, yb, yt, B=5000, seed=seed)
    # tuples: (mean, lo, hi)
...
def main():
    seed = 0  # set to your project seed if you want determinism
    rows = []
    for split in SPLITS:               # only "test"
        for m in TUNED_METHODS:
            r = compare_one(split, m, seed=seed)
            if r is not None:
                rows.append(r)
    if rows:
